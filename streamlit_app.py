...
def speech_to_text_js(key):
    ...
    function startRecognition(key) {{
        ...
        if (!SpeechRecognition) {{ status.innerText = "非対応ブラウザです"; return; }}
        const recognition = new SpeechRecognition();
        recognition.lang = 'ja-JP';
        recognition.onstart = () => {{ status.innerText = "認識中..."; status.style.color = "red"; }};
...
if st.button("👉 報告書を生成"):
    if not st.session_state['map_data']:
        ...
        with st.spinner("画像を生成中..."):
            ...
            y = draw_report_content(d, 300 + mh, all_keys, report)
